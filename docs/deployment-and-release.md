...
> 本轮状态（2026-09-07）：按用户明确要求未执行 Release、正式 ZIP、部署、安装、Windows CI 或发布门禁。下文保留为以后发布时使用的协议，不是通过记录。

V1 的自有运行时声明已包含 Headless 项目引用，以及 SharpCompress 0.50.4、SharpZipLib 1.4.2；预期 DLL 为 `LayerUnpackPlugin.Plugin.dll`、`LayerUnpackPlugin.Headless.dll`、`SharpCompress.dll` 和 `ICSharpCode.SharpZipLib.dll`。两库 MIT 许可作为 `ThirdPartyNotices` 资产声明。当前只做源码/锁文件/Debug 依赖检查，正式目录和 ZIP 内容需在发布阶段实际核对。

依赖放在 Headless 时，PackageReference 属于 Headless，Plugin 通过 ProjectReference 获得运行依赖，并在 Plugin 中声明 ManagedPluginPrivatePackage 资产归属；不需要为了重复引用而复制业务依赖。Build 1.1.2 不自动部署项目引用，因此 Plugin 的 CollectLayerUnpackHeadlessAsset 在 ResolveReferences 后从真实引用复制路径显式声明 Headless DLL。该声明已通过 Debug 求值和文件存在检查，未触发部署。
...
<!-- 2、3. src/LayerUnpackPlugin.Plugin/LayerUnpackPlugin.Plugin.csproj：引用并声明由插件携带 -->
<ItemGroup>
...
- 用 `dotnet list src/LayerUnpackPlugin.Plugin/LayerUnpackPlugin.Plugin.csproj package --include-transitive` 查看传递依赖；
...
```powershell
dotnet msbuild src/LayerUnpackPlugin.Plugin/LayerUnpackPlugin.Plugin.csproj `
  -t:DeployManagedPlugin `
  -p:Configuration=Debug `
  -p:ManagedPluginDeployRoot=C:\Path\To\Host\Controls
```

该目标只重建 `Controls/LayerUnpackPlugin`，不会清理 `Controls` 根目录或其他插件。需要给开发目录增加醒目标记时，
可覆盖目录名：
...
dotnet msbuild src/LayerUnpackPlugin.Plugin/LayerUnpackPlugin.Plugin.csproj `
  ...
  -p:Configuration=Debug `
  -p:ManagedPluginDeployRoot=C:\Path\To\Host\Controls `
  -p:ManagedPluginDirectoryName=LayerUnpackPlugin-Dev
```

`LayerUnpackPlugin-Dev` 只是文件夹名称，插件身份仍是 manifest 中的 `myavalonia.plugin.layer.unpack`。
...
dotnet msbuild src/LayerUnpackPlugin.Plugin/LayerUnpackPlugin.Plugin.csproj `
  -t:DeployManagedPlugin `
  -p:Configuration=Debug `
  -p:ManagedPluginDeployRoot=C:\Temp\LayerUnpackPlugin-Deploy\Controls
```

然后把整个 `C:\Temp\LayerUnpackPlugin-Deploy\Controls\LayerUnpackPlugin` 复制到 Host 的 `Controls` 下。目标叶子目录可以
改为 `LayerUnpackPlugin-Dev`，但必须遵守：

- 把插件目录作为一个整体替换，不要把新文件合并覆盖到旧目录，否则删除过的依赖可能残留；
- 同一个 Host 中只保留一份 `myavalonia.plugin.layer.unpack`，不能同时留下 `LayerUnpackPlugin` 和 `LayerUnpackPlugin-Dev`；
...
```text
src/LayerUnpackPlugin.Plugin/artifacts/managed-plugin-packages/
├─ LayerUnpackPlugin.Plugin-1.0.0-win-x64.zip
└─ LayerUnpackPlugin.Plugin-1.0.0-win-x64.manifest.json
```

ZIP 内保持 `Controls/LayerUnpackPlugin/` 布局；同名外置 `.manifest.json` 记录 ZIP 和文件摘要。正式交付时让二者
