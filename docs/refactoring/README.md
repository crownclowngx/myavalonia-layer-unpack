...
日期：2026-09-07。当前为 V1 本地候选加 R01 简洁交互：G0007 实现及本地验证完成，部分原生交互待验收，见[G0007 结果](G0007/result.md)。G0001–G0006 保留历史实现和证据；发布项仍待验收。
...
本地日志在忽略的 `artifacts/` 下便于复跑；当前摘要与验收状态保存在阶段结果中。G0006 验证时尚无基线提交；G0007 以 8ae2949 为基线，并用源码/配置/夹具摘要标识本轮工作区；后续 Git 提交保存验证快照，不改写原始门禁记录，不虚构正式包摘要。
