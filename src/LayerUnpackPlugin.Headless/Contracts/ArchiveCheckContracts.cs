...
public static class ArchiveDiagnosticAdvice
{
    public static string For(UnpackError code) => code switch
    {
        ...
        UnpackError.MissingVolume => "标准 .7z.001 分卷请补齐到同一文件夹，再重新识别此组；其他多卷格式请使用原工具。",
        UnpackError.MissingVolumeOrCorruptArchive => "无法区分缺卷与截断损坏，请核对所有卷与提供方摘要，再重新识别；不会自动等待或混入新卷。非标准名称与其他多卷格式请使用原工具。",
        UnpackError.InvalidVolumeSet => "整理重复编号或不一致的卷名，保持同一组编号规则后重新识别。",
