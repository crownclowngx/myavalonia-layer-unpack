...
MyAvaloniaManagement 的压缩与解压插件。提供独立 Headless 类库，以及“解压任务”“压缩任务”两个临时 Document。解压支持指定深度和当前批次候选密码共享；压缩将文件、文件夹和空目录合成一个普通 ZIP。

当前为 **R02 基础 ZIP 创建本地候选**：使用默认参数快速开始，清单和细节按需查看。最新实施与验证边界见[G0008 结果](docs/refactoring/G0008/result.md)，R01 与 V1 历史证据保留在 G0007 / G0006。本轮不使用 AIFLOW、Windows CI、Release、正式插件包、部署或发布门禁。
...
- **压缩**：固定普通 ZIP、标准 Deflate、UTF-8 名称；自动建议名称位置，父子输入合并、同名根编号，先临时写入再不覆盖提交。创建范围见[创建矩阵](docs/refactoring/G0008/format-support-matrix.md)，当前不含加密或分别打包。
...
后续发展见[压缩包工作台路线图](docs/roadmap/README.md)：R01 对应 G0007，R02 对应 G0008；R03–R08 仍可分别生成详细执行计划。
