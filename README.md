...
当前已接入 **G0015 标准 7z 分卷与按层串行解压**：同目录 `.7z.001` 等卷作为一个包，任意卷添加、整组校验、补密和显式重新识别共用同一规则。默认一层不变；当前能力与本地验证见 [G0015 结果](docs/refactoring/G0015/result.md)。7z 创建仍未开放；本轮不使用 AIFLOW、Windows CI 或发布门禁。
...
- **解压格式**：ZIP、7z、RAR4/5、UTF-8 TAR、GZip/BZip2/XZ；具体变体和限制以[当前支持矩阵](docs/refactoring/G0015/format-support-matrix.md)为准。RAR5 加密内容存在校验限制，结果节点会显示提示。
...
标准 `.7z.001` 分卷可直接添加任意卷；同一外层包解出同目录分卷后，开启两层即可继续读取。各层串行完成再发现下一层。缺卷补齐后点击“重新识别此组…”，保留成功外层，确认后执行新任务。详见 [当前矩阵](docs/refactoring/G0015/format-support-matrix.md) 和 [卷组契约](docs/refactoring/G0015/split-volume-contract.md)。
